...
router.post( '/', [auth, [ check('status', 'Status is required').not().isEmpty(), check('skills', 'Skills is required').not().isEmpty() ] ], 
    async (req, res) => {
        ...
        if(skills) {
            profileFields.skills = skills.toString().split(',').map(skill => skill.trim().toLowerCase());
        }
...
// @route     GET api/profile/github/:username no funcina
